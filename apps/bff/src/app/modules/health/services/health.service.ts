...
export class HealthService {
  ...
  checkMemoryHeap() {
    // Check if the heap memory usage is below 150MB
    return this.health.check([() => this.memory.checkHeap('memory_heap', 150 * 1024 * 1024)]);
  }

  /*
   * Check the readiness of the application by verifying connectivity to all dependent services.
   * This includes TCP and gRPC services, as well as Redis.
   */
  checkReadiness() {
    const tcpServices = [
      { key: TCP_SERVICES.INVOICE_SERVICE, configKey: 'TCP_CONFIG.TCP_INVOICE_SERVICE' },
      { key: TCP_SERVICES.PRODUCT_SERVICE, configKey: 'TCP_CONFIG.TCP_PRODUCT_SERVICE' },
      { key: TCP_SERVICES.USER_ACCESS_SERVICE, configKey: 'TCP_CONFIG.TCP_USER_ACCESS_SERVICE' },
      { key: TCP_SERVICES.AUTHORIZE_SERVICE, configKey: 'TCP_CONFIG.TCP_AUTHORIZE_SERVICE' },
      { key: TCP_SERVICES.PDF_GENERATOR_SERVICE, configKey: 'TCP_CONFIG.TCP_PDF_GENERATOR_SERVICE' },
      { key: TCP_SERVICES.MEDIA_SERVICE, configKey: 'TCP_CONFIG.TCP_MEDIA_SERVICE' },
    ];

    const grpcServices = [
      { key: GRPC_SERVICES.AUTHORIZER_SERVICE, configKey: 'GRPC_CONFIG.GRPC_AUTHORIZER_SERVICE' },
      { key: GRPC_SERVICES.USER_ACCESS_SERVICE, configKey: 'GRPC_CONFIG.GRPC_USER_ACCESS_SERVICE' },
    ];
